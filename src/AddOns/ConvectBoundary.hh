...
inline void ConvectParameterBoundary<TParameter, TParameterOld>::compute(int k) {
    ...
    if (normalq > 0) {
        ...
        for (int xyz = 0; xyz < TTraits::Lattice::NDIM; xyz++) {
            normalvelocity +=
                -normal[xyz] *
                Velocity<>::get<Lattice, Lattice::NDIM>(
                    DataType::getInstance().getNeighbor(DataType::getInstance().getNeighbor(k, normalq), normalq), xyz);
        ...
        for (int component = 0; component < TParameter::instances; component++) {
            TParameter::template get<Lattice>(k, component) =
                (TParameterOld::template get<Lattice>(k, component) +
                 normalvelocity *
                     TParameter::template get<Lattice>(DataType::getInstance().getNeighbor(k, normalq), component)) /
                (1 + normalvelocity);

            TParameter::template get<Lattice>(data.getNeighbor(k, Stencil::Opposites[normalq]), component) =
                (TParameterOld::template get<Lattice>(data.getNeighbor(k, Stencil::Opposites[normalq]), component) +
                 normalvelocity * TParameter::template get<Lattice>(k, component)) /
                (1 + normalvelocity);
        }
    }
}
