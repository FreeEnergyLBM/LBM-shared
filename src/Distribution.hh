...
struct Distribution_Base {
    ...
    std::vector<double> mv_OldDistribution; //!<Possibly unused vector storing the old distributions (at t_old=t_new-1) //This is required in the collision step
    ...
    //!Get a vector containing the distributions
    inline std::vector<double>& getEquilibrium() {
    ...
    //!Get a pointer to the the distribution at lattice point k and pointing in direction 0
    inline double* getEquilibriumPointer(const int k) {
        return &mv_EquilibriumDistribution[k *TStencil::Q];
    }

    //!Get const distribution value at a given index
    inline const double& getEquilibrium(const int idx) const {
        return mv_EquilibriumDistribution[idx];
    }

    //!Get distribution value at a given index
    inline double& getEquilibrium(const int idx) { 
        return mv_EquilibriumDistribution[idx];
    }
};
