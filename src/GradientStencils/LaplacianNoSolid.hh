...
inline double LaplacianCentralNoSolid::compute(const int direction, const int k, int num){
    ...
    for (int idx = 1; idx <traits::Stencil::Q; idx++) {
            ...
            if(!Geometry<typename traits::Lattice>::isSolid(DataType::getInstance().getNeighbors()[k * traits::Stencil::Q + idx])) laplaciansum +=  traits::Stencil::Weights[idx] * 2 * (parameter::template get<typename traits::Lattice>(DataType::getInstance().getNeighbors()[k * traits::Stencil::Q + idx],num) - parameter::template get<typename traits::Lattice>(k,num));
