...
class InputParameters{
    ...
    public:
        ...
        std::string& getParameterType(std::string name){
            return mVarTypes.at(name);
        }
        ...
        const std::string& getParameterType(std::string name) const {
            return mVarTypes.at(name);
        }
        ...
        void readInput(std::string fileName)
        {
            ...
            SetVar(mVarKeys,mVarTypes,seglist);                                  // Function which sets variables based on the ordering in the seglist vector
        }
        ...
        //===========================================================
        void SetVar(std::map<std::string, std::any>& Keys,
                    std::map<std::string,std::string>& Types,
                    const std::vector<std::string>& input){
                        
            if(input.size()%2!=0){
                std::cout<<"WARNING: Input file may not be constructed correctly. Ensure Variables are defined as: name=value and everything else is commented with % or #.\n\nYou can separate variables using spaces, tabs or new lines. The code is pretty lenient with spaces and tabs (will be ignored if between an equals sign and a name/value, will be ignored if repeated etc)."<<std::endl;
            }
            ...
            for(int i=0; i < (int)input.size(); i++) {
                if (i%2==0){                                                   // Take only even (including zero) indices as these correspond to variable names.
                    ...
                    if ( Keys.find(tempinput) == Keys.end() && mpi.rank==0 ) {                     // If the variable name is NOT found in the Keys map (variable names are automatically stored in this map if defined as a "simulation_parameter")
                        std::cout<<"Cannot find a variable named "
                                <<tempinput<<". This could mean the "
                    ...
                    // Else if the variable name is NOT found in the Types
                        // map
                    else if (Types.find(tempinput) == Types.end() && mpi.rank==0) {              // If the variable type is NOT found in the Keys map (variable type are automatically stored in this map if defined as a "simulation_parameter")
                        std::cout<<"WARNING: Cannot find type of variable "
                                <<tempinput<<". Check that names match"
                                " in \"VarKeys()\" and \"VarTypes\" in " 
                                "initialise_simulation.cc."<<std::endl;
                        ...
                        if(TYPE=="i"){                                                 // Integer
                            *std::any_cast<int*>(Keys[tempinput])=std::stoi(input[i+1]);                                  // Set that variable with the variable in the next entry in the vector
                        }
                        ...
                        if(TYPE=="NSt7__cxx1112basic_stringIcSt11char_traitsIc"        // String
                                "ESaIcEEE"){
                            *std::any_cast<std::string*>(Keys[tempinput])=input[i+1];
                        }

                        if (mpi.rank==0)   std::cout<<tempinput<<" = "
                                                    <<input[i+1]<<std::endl;
                    }
                }
            }
