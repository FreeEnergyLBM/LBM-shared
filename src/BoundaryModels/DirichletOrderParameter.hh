...
class DirichletOrderParameter : public BoundaryBase {
    public:
        ...
        DirichletOrderParameter() { this->setNodeID(4, true); } // TMP: Default NodeID warning
...
inline void DirichletOrderParameter::compute(TDistributionType& distribution, int k) {
    ...
    for (int idx = 0; idx < TTraits::Stencil::Q; idx++) {
        ...
        distribution.getDistributionPointer(distribution.streamIndex(k, idx))[idx] = -distribution.getPostCollisionDistribution(distribution.streamIndex(k, idx),distribution.getOpposite(idx)) + 2*TTraits::Stencil::Weights[idx]*ChemicalPotential<>::get<Lattice>(distribution.streamIndex(k, idx))/ TTraits::Stencil::Cs2*mInterfaceVal;
