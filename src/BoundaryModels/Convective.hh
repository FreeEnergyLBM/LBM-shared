...
template<class TTraits>
inline void Convective::runProcessor(int k) { //CHANGE THIS SO YOU DONT NEED TO COMMUNICATE
        ...
        for (int xyz = 0; xyz < TTraits::Lattice::NDIM; xyz++) {
            normalvelocity += -normal[xyz]*(Velocity<>::get<Lattice, Lattice::NDIM>(data.getNeighbor(data.getNeighbor(k,normalq),normalq),xyz));
...
class Convective2 : public BoundaryBase {
    public:
        ...
        inline void setVelocityCalculator(double (*v)(const double* distribution, ForceTuple& forcetuple,  const double& density, int xyz, int k)) {mVelocityCalculator=v;}

    private:

        TForceTuple mt_Forces;

        static double defaultVelocityCalculator(const double* distribution, ForceTuple& forcetuple, const double& density, int xyz, int k) { return 0; }

        double (*mVelocityCalculator)(const double* distribution, ForceTuple& forcetuple, const double& density, int xyz, int k) = &defaultVelocityCalculator;
...
inline void Convective2<TForceTuple>::compute(TDistributionType& distribution, int k) { //CHANGE THIS SO YOU DONT NEED TO COMMUNICATE
    ...
    for (int idx = 0; idx < Stencil::Q; idx++) {
        ...
        distribution.getDistributionPointer(distribution.streamIndex(k, normalq))[idx] = (distribution.getDistributionOldPointer(distribution.streamIndex(k, normalq))[idx]+normalvelocity*distribution.getDistributionPointer(distribution.streamIndex(distribution.streamIndex(k, normalq), normalq))[idx])/(1.0+normalvelocity);
