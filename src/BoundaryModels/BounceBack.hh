...
// Not different from BounceBack at the moment, but it is used in HLBM model and might
// become different in the future
class BounceBackHLBM : public BoundaryBase {
   public:
    template <class TTraits, class TDistributionType>
    inline void compute(TDistributionType& mDistribution, int k);
};
...
template <class TTraits, class TDistributionType>
inline void BounceBackHLBM::compute(TDistributionType& distribution, int k) {
    using Lattice = typename TTraits::Lattice;
