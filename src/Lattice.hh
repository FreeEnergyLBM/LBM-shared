 ...
 * Within these classes, we have fields for the lattice dimensions and information for the halo regions to be  communicated with MPI.
 * These classes also contain functions to communicate parameters.
 * Currently there are two lattice classes, one in which the lattice dimensions are known at compile time and another where they will be passed at runtime.
 */

/**
 * \brief This class will contain lattice information and handle parallelisation when the domain size is known at compile time.
 * \tparam TParallel Class to handle MPI parallelisation.
...
struct LatticeProperties{
    ...
    static constexpr int NDIM = 3 - (lx <= 1 || ly <= 1 || lz <=1 ) * (1 + ((lx <= 1 && ly <=1)||(lx <= 1 && lz <=1)||(ly <= 1 && lz <=1))); //!<Number of cartesian directions for the lattice.
    ...
    inline static int LXMPIOffset = 0, LYMPIOffset = 0, LZMPIOffset = 0; //!<Global coordinates for the start of local regions (excluding halo).
    ...
    inline static int Face[6] = {0, 0, 0, 0, 0, 0}; //!<Size of one slice parallel region. For 1D parallelisation only Face[0] is used.
    ...
    static std::map<std::type_index,bool> alreadycommunicatedparameter; //!<Map of parameter type to true if it has been communicated and false if not.

    enum {stream = 0, all = 1, allequilibrium = 2, allold = 3};

    static std::unordered_map<std::pair<int,double*>,bool,pair_hash> alreadycommunicateddistribution; //!<Map of pair containing distribution communication type and pointer to distribution array to true if it has already been communicated and false if not.
    ...
    /**
     * \brief This function communicates a parameter adjacent to the parallel boundary into the halo regions of the parameter in all neighboring lattices.
     * \tparam TParameter type of the parameter.
     * \param obj object of the parameter.
     */
    ...
    static void communicateDistribution(TDistribution& obj) {
        ...
        #pragma omp master
        {
        // If the distribution has not already been communicated with this communication type or does not exist in the map.
    ...
    /**
     * \brief This function communicates all distributions adjacent to the parallel boundary into the halo regions of the distribution array in all neighboring lattices.
     * \tparam TDistribution type of the distribution.
     * \param obj object of the distribution.
     */
    template<class TDistribution>
    ...
    /**
     * \brief This function communicates all equilibrium distributions adjacent to the parallel boundary into the halo regions of the distribution array in all neighboring lattices.
     * \tparam TDistribution type of the distribution.
     * \param obj object of the distribution.
     */
    template<class TDistribution>
    ...
    /**
     * \brief This function communicates all distributions from the previous timestep adjacent to the parallel boundary into the halo regions of the distribution array in all neighboring lattices.
     * \tparam TDistribution type of the distribution.
     * \param obj object of the distribution.
     */
    template<class TDistribution>
...
template<class TParallel, int lx, int ly, int lz>
std::unordered_map<std::pair<int,double*>,bool,pair_hash> LatticeProperties<TParallel,lx,ly,lz>::alreadycommunicateddistribution;

/**
 * \brief This class will contain lattice information and handle parallelisation when the domain size is not known at compile time.
 * \tparam TParallel Class to handle MPI parallelisation.
 * \tparam TNDIM Number of cartesian lattice directions.
 */
...
struct LatticePropertiesRuntime {
    ...
    /**
     * \brief This function communicates all eqiuilibrium distributions adjacent to the parallel boundary into the halo regions of the distribution array in all neighboring lattices.
     * \tparam TDistribution type of the distribution.
     * \param obj object of the distribution.
     */
    template<class TDistribution>
...
template<class TParallel, int TNDIM>
std::unordered_map<std::pair<int,double*>,bool,pair_hash> LatticePropertiesRuntime<TParallel,TNDIM>::alreadycommunicateddistribution;
