...
void X_Parallel<stencil,num_neighbors>::communicate(parameter& obj){
    ...
    MPI_Isend(&obj.getParameter()[GETPROPERTIES().m_N*obj.m_Num-(num_neighbors+1)*GETPROPERTIES().m_LY*GETPROPERTIES().m_LZ], num_neighbors*GETPROPERTIES().m_LY*GETPROPERTIES().m_LZ*obj.m_Num, mpi_get_type<typename parameter::ParamType>(), m_RightNeighbor, 0, MPI_COMM_WORLD, &comm_request[0]);

    MPI_Isend(&obj.getParameter()[num_neighbors*GETPROPERTIES().m_LY*GETPROPERTIES().m_LZ*obj.m_Num], num_neighbors*GETPROPERTIES().m_LY*GETPROPERTIES().m_LZ*obj.m_Num, mpi_get_type<typename parameter::ParamType>(), m_LeftNeighbor, 1, MPI_COMM_WORLD, &comm_request[1]);

    MPI_Irecv(&obj.getParameter()[GETPROPERTIES().m_N*obj.m_Num-num_neighbors*GETPROPERTIES().m_LY*GETPROPERTIES().m_LZ*obj.m_Num], num_neighbors*GETPROPERTIES().m_LY*GETPROPERTIES().m_LZ*obj.m_Num, mpi_get_type<typename parameter::ParamType>(), m_RightNeighbor, 1, MPI_COMM_WORLD, &comm_request[0]);

    MPI_Irecv(&obj.getParameter()[0], num_neighbors*GETPROPERTIES().m_LY*GETPROPERTIES().m_LZ*obj.m_Num, mpi_get_type<typename parameter::ParamType>(), m_LeftNeighbor, 0, MPI_COMM_WORLD, &comm_request[1]);
    ...
    if (CURPROCESSOR<NUMPROCESSORS){
        ...
        for(int idx=1; idx<stencil::Q;idx++){
            ...
            if(leftorright==-1){
                ...
                MPI_Isend(&obj.getDistribution()[(num_neighbors-1)*GETPROPERTIES().m_LY*GETPROPERTIES().m_LZ*stencil::Q+idx],1,DistributionVector,m_LeftNeighbor,id,MPI_COMM_WORLD,&comm_dist_request[id]);
                id+=1;

            }
            else if(leftorright==1){
            
                MPI_Isend(&obj.getDistribution()[GETPROPERTIES().m_N*stencil::Q-(num_neighbors)*GETPROPERTIES().m_LY*GETPROPERTIES().m_LZ*stencil::Q+idx],1,DistributionVector,m_LeftNeighbor,id,MPI_COMM_WORLD,&comm_dist_request[id]);
                id+=1;
        ...
        for(int idx=1; idx<stencil::Q;idx++){
            ...
            if(leftorright==-1){
                ...
                MPI_Irecv(&obj.getDistribution()[GETPROPERTIES().m_N*stencil::Q-(num_neighbors+1)*GETPROPERTIES().m_LY*GETPROPERTIES().m_LZ*stencil::Q+idx],1,DistributionVector,m_RightNeighbor,id,MPI_COMM_WORLD,&comm_dist_request[id]);
                id+=1;

            }
            else if(leftorright==1){
            
                MPI_Irecv(&obj.getDistribution()[(num_neighbors)*GETPROPERTIES().m_LY*GETPROPERTIES().m_LZ*stencil::Q+idx],1,DistributionVector,m_RightNeighbor,id,MPI_COMM_WORLD,&comm_dist_request[id]);
                id+=1;
