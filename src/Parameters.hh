...
class ParameterSingleton {
   public:
    ...
    static inline Parameter<TObj, TLattice, T, TNumPrefactor * TNum> &getInstance() {
        static Parameter<TObj, TLattice, T, TNumPrefactor * TNum> instance;
        return instance;
    };
    ...
    static inline T &get(int idx1, int idx2, int idx3) {  // Returns const parameter at index idx
        // MAKE MORE EFFICIENT!!!
        // static Parameter<TObj,TLattice,T,TNumPrefactor*TNum>& instance = getInstance<TLattice,TNum>();
        return getInstance<TLattice, TNum>()
            .mv_Parameter[idx1 * instances * TNum + (instances > 1) * idx2 * TNum + (TNum > 1) * idx3];
    }
...
#pragma omp critical
        {
            if (!getInstance<TLattice, TNum>().mmInitialised.count(idx1 * instances * TNum +
                                                                   (instances > 1) * idx2 * TNum + (TNum > 1) * idx3)) {
                getInstance<TLattice, TNum>()
                    .mv_Parameter[idx1 * instances * TNum + (instances > 1) * idx2 * TNum + (TNum > 1) * idx3] = val;
                getInstance<TLattice, TNum>().mmInitialised.insert(
                    {idx1 * instances * TNum + (instances > 1) * idx2 * TNum + (TNum > 1) * idx3, true});
            } else {
                getInstance<TLattice, TNum>().mmInitialised.erase(idx1 * instances * TNum +
                                                                  (instances > 1) * idx2 * TNum + (TNum > 1) * idx3);
            }
        }
    }
...
#pragma omp critical
        {
            if (!getInstance<TLattice, TNum>().mmInitialised.count(idx1 * instances * TNum + idxlambda<TNum>(idx2))) {
                ...
                getInstance<TLattice, TNum>().mmInitialised.insert(
                    {idx1 * instances * TNum + idxlambda<TNum>(idx2), true});
            } else {
    ...
    static void set(T val) {
        for (int k = TLattice::HaloSize; k < TLattice::N - TLattice::HaloSize; k++) {
            if constexpr (idx2 == 0)
                initialise<TLattice, TNum>(val, k, idx1);
            else
                initialise<TLattice, TNum>(val, k, idx1, idx2);
        }
    }
    ...
    static void set(T (*condition)(const int)) {
        for (int k = TLattice::HaloSize; k < TLattice::N - TLattice::HaloSize; k++) {
            if constexpr (idx2 == 0)
                initialise<TLattice, TNum>(condition(k), k, idx1);
            else
        ...
        for (int k = TLattice::HaloSize; k < TLattice::N - TLattice::HaloSize; k++) {
            if (condition(k)) {
                if constexpr (idx2 == 0)
                    ...
                    initialise<TLattice, TNum>(false_val, k, idx1, idx2);
            } else {
                if constexpr (idx2 == 0)
                    initialise<TLattice, TNum>(false_val, k, idx1);
                else
                    initialise<TLattice, TNum>(false_val, k, idx1, idx2);
            }
        }
