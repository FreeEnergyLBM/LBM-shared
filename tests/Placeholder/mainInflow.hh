...
int initBoundary(const int k) {
    ...
    if ((true||xx<postwidth/2||xx>lx-postwidth/2)&&yy<ly/4) return 1;
    //if (yy <= 5) return 1;
...
using traithumid = DefaultTraitHumidity<Lattice>::SetStencil<D2Q9>:: template SetBoundary<Dirichlet,Dirichlet,ExtrapolationOutflow,Refill<Humidity<>>,FreeSlip>;
...
template<class TLattice>
using DefaultTraitPressureLeeHumidityInflow = typename DefaultTraitPressureLee2<TLattice> :: template AddPreProcessor<Swapper<Velocity<>, VelocityOld<>, TLattice::NDIM>,chempotgradients,SetParameterOld<Pressure<>,PressureOld<>>,SetParameterOld<Density<>,DensityOld<>>,SetParameterOld<OrderParameter<>,OrderParameterOld<>>>
                                                                                   //:: template AddForce<EvaporationPressureSource<EvaporationSourceMethod>>
                                                                                   :: template SetBoundary<std::tuple<std::tuple<FreeSlip,BounceBack>,std::tuple<VelocityInflowVariable,Convective2<typename DefaultTraitPressureLee2<Lattice>::Forces>>>>;//,std::tuple<Temp0Outflow>>>;,ExtrapolationOutflow
...
auto initPressure(){
    ...
    pressure.template getBoundary<BounceBack,0>().setInterfaceID({1,2});
    pressure.template getBoundary<FreeSlip,0>().setInterfaceID({11});
    pressure.template getBoundary<VelocityInflowVariable,1>().setInterfaceID({3});
...
template<typename TTrait = typename DefaultTraitFlowField<Lattice> :: template SetBoundary<std::tuple<std::tuple<FreeSlip,BounceBack>,std::tuple<VelocityInflowVariable,Convective2<typename DefaultTraitFlowField<Lattice>::Forces>>>>>
auto initFlowField(){
...
template<class TLattice>
using DefaultTraitBinaryLeeHumidityInflow = typename DefaultTraitBinaryLee<TLattice> :: template SetPostProcessor<orderparamgradients,ChemicalPotentialCalculatorBinaryLee>//,NoFluxSolid<OrderParameter<>>,NoFluxSolid<Pressure<>>>
                                                                               :: template AddPreProcessor<GradientsMultiStencil<Pressure<>, CentralXYZBounceBack, CentralQBounceBack,
                                                                                                                                MixedXYZBounceBack, MixedQBounceBack>,MassLossCalculatorInterpolated>
                                                                               :: template AddForce<EvaporationPhaseSource<EvaporationSourceMethod>>
                                                                               :: template SetBoundary<std::tuple<std::tuple<Convective>,std::tuple<FreeSlip>>>;
...
auto initBinary(){
    ...
    binary.template getBoundary<FreeSlip,1>().setInterfaceID({1,3});
...
void AfterEquilibration(int eqsteps,T& model) {
    if (eqsteps>equilibriumtimesteps) model.template getPreProcessor<MassLossCalculatorInterpolated>().setDiffusivity(diffusivity);
}