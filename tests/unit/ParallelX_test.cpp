  ...
  for (int i=0; i<neighbors; ++i)
    for (int y=0; y<ysize; ++y)
      for(int z=0; z<zsize; ++z)
        ...
        {
          int k = (i==0) ? nodeLeft+(z+y*zsize)*Q+j : nodeRight+(z+y*zsize)*Q+j; // element in the distribution (with directions)
          double value = ((neighbors*mpi.rank+i)*ysize*zsize+(z+y*zsize))*Q+j; // the value in the element of the distribution
...
// 2. SETTING THE EXPECTED VECTOR WITH EXPECTED VALUES AFTER COMMUNICATION
  ...
  for(int y=0; y<ysize; ++y)
    for(int z=0; z<zsize; ++z)
    {
      ...
      nodeRightArray[k+2] += (mpi.rank<mpi.size-1) ?  ysize*zsize*Q : (1-2*mpi.size)*ysize*zsize*Q;
    }
...
// 4. RETRIEVING DATA FROM DISTRIBUTION AFTER COMMUNICATION
  std::vector<double> distrNodeLeft(distr.getDistributionPointer(width*ysize*zsize), distr.getDistributionPointer((width+1)*ysize*zsize));
  std::vector<double> distrNodeRight(distr.getDistributionPointer((xExt-width-1)*ysize*zsize), distr.getDistributionPointer((xExt-width)*ysize*zsize));
...
// 2. SETTING THE EXPECTED VECTOR WITH EXPECTED VALUES AFTER COMMUNICATION
  ...
  for(int y=0; y<ysize; ++y)
    for(int z=0; z<zsize; ++z)
    {
      ...
      nodeRightArray[k+6] += (mpi.rank<mpi.size-1) ? ysize*zsize*Q : (1-2*mpi.size)*ysize*zsize*Q;
      nodeRightArray[k+8] = distr.getDistribution(nodeRight+k+8);
      nodeRightArray[k+8] += (mpi.rank<mpi.size-1) ? ysize*zsize*Q : (1-2*mpi.size)*ysize*zsize*Q;
    }
...
// 2. SETTING THE EXPECTED VECTOR WITH EXPECTED VALUES AFTER COMMUNICATION
  ...
  for(int y=0; y<ysize; ++y)
    for(int z=0; z<zsize; ++z)
    {
      ...
      nodeRightArray[k+10] += (mpi.rank<mpi.size-1) ? ysize*zsize*Q : (1-2*mpi.size)*ysize*zsize*Q;
      nodeRightArray[k+12] = distr.getDistribution(nodeRight+k+12);
      nodeRightArray[k+12] += (mpi.rank<mpi.size-1) ? ysize*zsize*Q : (1-2*mpi.size)*ysize*zsize*Q;
      nodeRightArray[k+13] = distr.getDistribution(nodeRight+k+13);
      nodeRightArray[k+13] += (mpi.rank<mpi.size-1) ? ysize*zsize*Q : (1-2*mpi.size)*ysize*zsize*Q;
    }
