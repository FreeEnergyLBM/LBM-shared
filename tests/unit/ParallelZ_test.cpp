  ...
  for(int x=0; x<xsize; ++x)
    for(int y=0; y<ysize; ++y)
      for(int z=width; z<zExt-width; ++z)
      {
        ...
        density[i] = i + y*(ZSIZE-zExt) + x*ysize*(ZSIZE-zExt) + (mpi.rank*(zExt-2*width)-width) + 1;
        //      z + y*ZSIZE + x*ysize*ZSIZE + (mpi.rank*(zExt-2*width)-width) + 1
        // or:  z + y*zExt + x*ysize*zExt + y*(ZSIZE-zExt) + x*ysize*(ZSIZE-zExt) + (mpi.rank*(zExt-2*width)-width) + 1
      }
  ...
  for (int i=0; i<neighbors; ++i)
    for (int x=0; x<xsize; ++x)
      for(int y=0; y<ysize; ++y)
      {
        ...
        for (int j=0; j<Q; ++j)
        {
          int k = (i==0) ? nodeFront+l+j : nodeBack+l+j; // element k in the distribution (with directions)
          double value = ((neighbors*mpi.rank+i)*xsize*ysize+(y+x*ysize))*Q+j; // the value in the element of the distribution
...
// 2. SETTING THE EXPECTED VECTOR WITH EXPECTED VALUES AFTER COMMUNICATION
  ...
  for(int x=0; x<xsize; ++x)
    for(int y=0; y<ysize; ++y)
    {
      ...
      nodeBackArray[k+6] += (mpi.rank<mpi.size-1) ? xsize*ysize*Q : (1-2*mpi.size)*xsize*ysize*Q;
      nodeBackArray[k+8] = distr.getDistribution(nodeBack+l+8);
      nodeBackArray[k+8] += (mpi.rank<mpi.size-1) ? xsize*ysize*Q : (1-2*mpi.size)*xsize*ysize*Q;
      nodeBackArray[k+9] = distr.getDistribution(nodeBack+l+9);
      nodeBackArray[k+9] += (mpi.rank<mpi.size-1) ? xsize*ysize*Q : (1-2*mpi.size)*xsize*ysize*Q;
      nodeBackArray[k+12] = distr.getDistribution(nodeBack+l+12);
      nodeBackArray[k+12] += (mpi.rank<mpi.size-1) ? xsize*ysize*Q : (1-2*mpi.size)*xsize*ysize*Q;
      nodeBackArray[k+14] = distr.getDistribution(nodeBack+l+14);
      nodeBackArray[k+14] += (mpi.rank<mpi.size-1) ? xsize*ysize*Q : (1-2*mpi.size)*xsize*ysize*Q;
    }
