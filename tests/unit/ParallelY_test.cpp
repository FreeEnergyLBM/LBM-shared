  ...
  for (int i=0; i<neighbors; ++i) {
    for (int x=0; x<xsize; ++x)
      for(int z=0; z<zsize; ++z)
      {
        ...
        {
          int k = (i==0) ? nodeBottom+l+j : nodeTop+l+j; // element k in the distribution (with directions)
          double value = ((neighbors*mpi.rank+i)*xsize*zsize+(z+x*zsize))*Q+j; // the value in the element of the distribution
...
// 2. SETTING THE EXPECTED VECTOR WITH EXPECTED VALUES AFTER COMMUNICATION
  ...
  for(int x=0; x<xsize; ++x)
    for(int z=0; z<zsize; ++z)
    {
      ...
      nodeTopArray[k+4] += (mpi.rank<mpi.size-1) ? xsize*zsize*Q : (1-2*mpi.size)*xsize*zsize*Q;
    }
...
// 2. SETTING THE EXPECTED VECTOR WITH EXPECTED VALUES AFTER COMMUNICATION
  ...
  for(int x=0; x<xsize; ++x)
    for(int z=0; z<zsize; ++z)
    {
      ...
      nodeTopArray[k+6] += (mpi.rank<mpi.size-1) ? xsize*zsize*Q : (1-2*mpi.size)*xsize*zsize*Q;
      nodeTopArray[k+7] = distr.getDistribution(nodeTop+l+7);
      nodeTopArray[k+7] += (mpi.rank<mpi.size-1) ? xsize*zsize*Q : (1-2*mpi.size)*xsize*zsize*Q;
    }
...
// 2. SETTING THE EXPECTED VECTOR WITH EXPECTED VALUES AFTER COMMUNICATION
  ...
  for(int x=0; x<xsize; ++x)
    for(int z=0; z<zsize; ++z)
    {
      ...
      nodeTopArray[k+8] += (mpi.rank<mpi.size-1) ? xsize*zsize*Q : (1-2*mpi.size)*xsize*zsize*Q;
      nodeTopArray[k+10] = distr.getDistribution(nodeTop+l+10);
      nodeTopArray[k+10] += (mpi.rank<mpi.size-1) ? xsize*zsize*Q : (1-2*mpi.size)*xsize*zsize*Q;
      nodeTopArray[k+11] = distr.getDistribution(nodeTop+l+11);
      nodeTopArray[k+11] += (mpi.rank<mpi.size-1) ? xsize*zsize*Q : (1-2*mpi.size)*xsize*zsize*Q;
      nodeTopArray[k+14] = distr.getDistribution(nodeTop+l+14);
      nodeTopArray[k+14] += (mpi.rank<mpi.size-1) ? xsize*zsize*Q : (1-2*mpi.size)*xsize*zsize*Q;
    }
